...
export default function LoginPage() {
  ...
  return (
    <AuthShell>
      <h2 style={{ fontSize: 20, fontWeight: 700, color: C.text, margin: "0 0 4px" }}>Welcome back</h2>
      <p style={{ fontSize: 13.5, color: C.textMuted, margin: "0 0 26px" }}>Log in to manage your attendance, salary and expenses.</p>
      <ErrorBlock message={error} />
      <form onSubmit={handleSubmit}>
        <Field label="Email" icon={Mail} type="email" placeholder="you@company.com" value={email} onChange={(e) => setEmail(e.target.value)} />
        <PasswordField label="Password" value={password} onChange={(e) => setPassword(e.target.value)} />
        <Button full type="submit" icon={ArrowRight} disabled={submitting}>{submitting ? "Logging in…" : "Log In"}</Button>
      </form>
      ...
      <Link to="/register" style={{ textDecoration: "none" }}>
        <Button full variant="outline">Create an account</Button>
      </Link>
    </AuthShell>
  );
}
